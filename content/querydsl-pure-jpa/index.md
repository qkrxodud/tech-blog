---
title: "순수 JPA와 함께 쓰기"
tags: ["Querydsl","JPA","동적 쿼리","리포지토리","Spring Boot"]
...
> 참고: 동시성 문제는 걱정하지 않아도 됩니다. 왜냐하면 여기서 스프링이 주입해주는 엔티티 매니저는 실제 동작 시점에 진짜 엔티티 매니저를 찾아주는 프록시용 가짜 엔티티 매니저이기 때문입니다. 이 가짜 엔티티 매니저는 실제 사용 시점에 트랜잭션 단위로 실제 엔티티 매니저(영속성 컨텍스트)를 할당해줍니다.
...
> 참고: @QueryProjection을 사용하면 해당 DTO가 Querydsl에 의존하게 됩니다. 이런 의존이 싫다면 해당 애노테이션을 제거하고, `Projections.bean()`, `fields()`, `constructor()`을 사용하면 됩니다.
...
**참고: where 절에 파라미터 방식을 사용하면 조건을 재사용할 수 있습니다.**
...
테스트는 기존 application.yml을 복사해서 다음 경로로 복사하고, 프로파일을 test로 수정합니다.
...
이렇게 분리하면 main 소스코드와 테스트 소스 코드 실행 시 프로파일을 분리할 수 있습니다.
